...
const ExtendProfileCard = ({extenddata}) => {
  return (
    <>
      <div className='flex flex-col border-2 border-[yellow]  md:w-1/5 hover:shadow-[#cccb78] shadow-2xl  hover:bg-right-bottom cursor-pointer transition ease-in-out delay-20 
        ...
        <div className='flex text-white font-Boogaloo flex-col mt-14 items-center py-4 gap-2'>
           <h1 className='text-2xl text-gfgmaincolor font-bold'>{extenddata.name}</h1>
           ...
           <NavLink to={extenddata.github} target='_blank'>
              <BsGithub className='fill-[#838383]  hover:fill-[#ffffff] cursor-pointer' />
           </NavLink>
           </div>
        </div>
      </div>
